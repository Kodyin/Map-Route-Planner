...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for (int i = 0;  i<numVertices; i ++) {
			ArrayList<Integer> outE = (ArrayList<Integer>) getNeighbors(i);
			ArrayList<Integer> inE  = (ArrayList<Integer>) getInNeighbors(i);
			int outCount = 0, inCount = 0;
			for (int j = 0;  j< numVertices; j++)  if (outE.contains((Integer)j)) outCount++;
			for (int j = 0;  j< numVertices; j++)  if (inE.contains((Integer)j)) inCount++;
			DS.add((Integer) inCount+outCount);
			Collections.sort(DS);
	...
	public static void main (String[] args) {
		...
		System.out.println("\n****");
		//System.out.println(graphFromFile.getDistance2(1).get(1));
